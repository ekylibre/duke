...
| `user_message` | `id`, `text` | Phrase utilisateur (texte tapé, STT navigateur, ou transcript renvoyé par `POST /api/v1/stt/transcribe`). |
...
| D7 | `ELEVATOR=header` dans l'env Ekylibre dev/prod | Ekylibre / Infra | ✅ Résolu via élevator hybride `Apartment::Elevators::Header < Apartment::Elevators::Subdomain` (`config/initializers/apartment.rb`) : X-Tenant prioritaire, fallback subdomain — browser et Duke cohabitent sur le même container |
...
| 9 | Stabilisation Ekylibre live | Mapper aligné sur les contraintes Rails/Procedo : (a) `provider: {vendor, name}` envelope ; (b) champs intervention à plat (pas sous `intervention:`) car `super.permit(common_params_to_permit)` filtre la clé wrapper ; (c) `*_attributes` en Array form non-vide (ComputedParameters appelle `.each_with_index`) avec collections vides omises ; (d) canonicalisation `procedure_name` via lexique (« labour » → `generic_tillage`) + ré-attache `raw_text` en `description` ; (e) `working_periods_attributes` toujours émis (synthèse `stopped_at` si absent — l'interactor compare avec `nil` et crashe). `ProcedureRegistry` (`procedure_registry.py`) hydrate le lexique au 1ᵉʳ auth via `GET /api/v2/procedures` (133 procédures) ; `parameter_name_for_role` choisit le bon `reference_name` par slot Procedo (`cultivation` pour `generic_tillage`). Patches upstream Ekylibre : `ELEVATOR=header` + élevator hybride Header < Subdomain (cohabitation browser/Duke), `_parameter.json.jbuilder` guard sur `handlers.values`, `procedures_controller#index` avec `procedure_action` (collision Rails `params[:action]`) + `render 'index'`, `Procedo::Engine::Interpreter` migré de `Language::*` vers `Nodes::*` (bug autoload `ActorPresenceTest`) |
| 10 | NER agricole entraîné | Corpus enrichi à 267 phrases / 401 spans depuis input terrain ; 2 labels ajoutés : `DUKE_WORKER` (opérateurs/doers), `DUKE_TOOL` (équipements). Dates et durées **restent hors NER** — déjà extraites par `nlu/temporal.py`. `train_ner.py` retire les pipes inutiles (parser, tagger, morphologizer, attribute_ruler, lemmatizer) avant `nlp.initialize` pour éviter l'init du lemmatizer (E955 `spacy-lookups-data`). Stage `trainer` du Dockerfile installe `fr_core_news_lg`, exécute le CLI, bake le modèle dans `/app/models/duke-ner` ; `pipeline.py` détecte le chemin baked en fallback automatique. Métriques live : F1 global **0.943** ; per-label PRODUCT 1.00, PARCEL 0.97, WORKER 0.96, QUANTITY 0.92, TOOL 0.91, PROCEDURE 0.91. CLI `corpus_stats` ajouté pour validation pré-train (label distribution, span alignment) |
| 11 | Whisper STT serveur (fallback) | Endpoint `POST /api/v1/stt/transcribe` (multipart `audio` + headers `Authorization: simple-token` / `X-Tenant`) — auth réutilise `EkylibreApiClient.validate_token()` avant de déclencher le modèle. Backend `faster-whisper` (CTranslate2, `small` fr-FR, `int8` CPU par défaut) chargé **lazy** au 1ᵉʳ appel via `WhisperService` (`asyncio.Lock` pour la 1ʳᵉ instanciation, `asyncio.to_thread` pour la transcription bloquante). Service toujours instancié au boot — gating sur `ENABLE_SERVER_STT` (503 si désactivé) ; la dépendance `faster-whisper` est en extra (`uv sync --extra stt`) pour éviter de la pousser dans l'image runtime quand non utilisée. Côté widget : `DukeWidgetController#show` expose `stt_server_enabled` + `stt_url` (dérivé de `DUKE_WS_URL` ou `DUKE_HTTP_URL`) ; le bouton micro tente Web Speech API d'abord, puis bascule vers `MediaRecorder` (Opus/WebM) → POST → injection du transcript dans le textarea (le pipeline NLU n'a aucun chemin spécifique au STT, il voit toujours du texte). Métriques `duke_stt_requests_total{outcome}` (ok / disabled / auth_invalid / auth_tenant / upstream_unavailable / too_large / empty / backend_unavailable / backend_error) |

**Couverture tests** : 117 unit + integration testcontainers par défaut + 6 e2e opt-in contre Ekylibre tournant + 1 smoke training NER opt-in (`RUN_NER_TRAINING=1`) + 1 smoke STT réel opt-in (`RUN_STT_SMOKE=1`, télécharge ~150-500 Mo au 1ᵉʳ run). Le widget JS n'a pas de suite Jest — validation manuelle dans Chrome/Edge (Web Speech API natif) et Firefox (chemin server-STT quand `ENABLE_SERVER_STT=true`).

**CI** (`.github/workflows/ci.yml`) : 3 jobs (lint ruff, pytest, build image). Image Docker buildée sur PR + push, poussée vers GHCR uniquement sur `main`. Cache Buildx GHA réutilise le download `fr_core_news_lg` (~545 Mo) et la layer de training entre runs.

## 12. Pistes pour la suite

- **NER agricole transformer** : passer `fr_core_news_lg` → `fr_dep_news_trf` une fois le corpus enrichi suffisamment par retours utilisateurs réels (le baseline F1 0.94 actuel suffit en MVP, gain transformer serait incrémental).
- **STT serveur — Whisper GPU / Voxtral** : monter `WHISPER_DEVICE=cuda` + `WHISPER_COMPUTE_TYPE=float16` quand un nœud GPU est disponible (latence ~3× sur CPU `int8` actuel), ou évaluer Mistral Voxtral si la souveraineté FR via API devient acceptable.
